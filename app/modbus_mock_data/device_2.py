...
DEVICE = DeviceSpec(
    ...
    holding_registers={
        ...
        3000: RegisterSpec(base=4800, min=3800, max=5600),  # Pack voltage ×10 V (380–560 V)
        3001: RegisterSpec(base=500, min=0, max=1500),  # Charge/discharge current ×10 A (0–150 A)
        3002: RegisterSpec(base=280, min=150, max=600),  # Pack temperature ×10 °C (15–60 °C)
        3003: RegisterSpec(base=750, min=0, max=1000),  # State of charge ×10 % (0–100 %)
        3004: RegisterSpec(base=5000, min=0, max=10000),  # Inverter output power W (0–10 kW)
    ...
    input_registers={
        ...
        4000: RegisterSpec(base=3200, min=2800, max=4200),  # Cell voltage min ×1 mV (2800–4200 mV, LFP)
        4001: RegisterSpec(base=3300, min=2800, max=4200),  # Cell voltage max ×1 mV
        4002: RegisterSpec(base=3250, min=2800, max=4200),  # Cell voltage avg ×1 mV
        4003: RegisterSpec(base=2500, min=0, max=5000),  # Total charge today ×10 Wh (0–500 Wh)
        4004: RegisterSpec(base=2000, min=0, max=5000),  # Total discharge today ×10 Wh (0–500 Wh)

        # 4005–4008 not defined — absent from register map.

        # ── Enum registers (4009–4011) — valid range 1–N ────────────────────────
        # 1=Init 2=Idle 3=Charging 4=Discharging 5=Balancing 6=Fault
        4009: RegisterSpec(base=4, min=1, max=6),
        # 1=LFP 2=NMC 3=NCA 4=LTO
        4010: RegisterSpec(base=1, min=1, max=4),
        # 1=Normal 2=Warm 3=Hot 4=Critical
        4011: RegisterSpec(base=1, min=1, max=4),
