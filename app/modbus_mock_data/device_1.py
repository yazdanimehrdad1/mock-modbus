...
HOLDING_REGISTERS: dict[int, dict[str, int]] = {
    ...
    1402: {"base": 3,     "min": 3,     "max": 3},     # active MPPT tracker count (fixed: 3 strings)
    1411: {"base": 2819,  "min": 2537,  "max": 3101},  # total DC input power from PV array (W)

    # ── Lifetime energy counters (32-bit high word; near 16-bit max for large site) ──
    1403: {"base": 63745, "min": 57370, "max": 65535},  # lifetime energy exported, high word (Wh)
    1404: {"base": 65529, "min": 58976, "max": 65535},  # total reactive energy delivered, high word (VARh)
    1405: {"base": 63949, "min": 57554, "max": 65535},  # total apparent energy delivered, high word (VAh)
    ...
    # ── AC output — per-phase grid voltages ─────────────────────────────────────
    ...
    # ── AC output — per-phase inverter output voltages (slightly above grid to push power) ──
    ...
    # ── Power quality ────────────────────────────────────────────────────────────
    1422: {"base": 35,    "min": 32,    "max": 39},    # voltage THD (×10, 3.5 %)
